...
- ADR-(Actores)-13: Eliminación del patrón Builder en Receptionist.
- ADR-06 (Dominio): Ubicación del patrón Builder en la entidad Dentist.
- ADR-18 (Dominio): Simplificación de jerarquía de excepciones.

---

### 📌 ¿Aplica realmente este patrón en este escenario?
Sí, aquí **sí aplica** porque:
- Appointment es un agregado **rico en reglas de negocio** y con múltiples atributos obligatorios.
- El *Builder* permite validar invariantes en un solo lugar (`build()`), evitando duplicación en el servicio.
- El servicio de dominio (`AppointmentSchedulingService`) necesita construir citas de forma clara y segura tras coordinar otros agregados.
- Receptionist no requería esta flexibilidad, pero Appointment sí, por lo que la excepción está justificada.
