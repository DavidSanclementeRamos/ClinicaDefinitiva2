...
public final class Company {
    ...
    private Company(
        ...
        if (Objects.isNull(typePerson)) {
            throw new DomainAggregateException(ErrorCatalog.ERR_COMPANY_MISSING_PERSON_TYPE, EntityContext.COMPANY);
        }
    ...
    public void updateStatus(CompanyStatus newStatus) {
        ...
        if (Objects.equals(this.status, CompanyStatus.of(CompanyStatus.Status.INACTIVE)) &&
            ...
            throw new BusinessRuleViolationException(ErrorCatalog.ERR_COMPANY_CANNOT_REACTIVATE_DIRECTLY, EntityContext.COMPANY);
        }
    ...
    private void ensureEditable() {
        if (!status.isEditable()) {
            throw new BusinessRuleViolationException(ErrorCatalog.ERR_COMPANY_NOT_EDITABLE, EntityContext.COMPANY);
        }
    }

    private void validateIncorporationDate(LocalDate date) {
        if (Objects.isNull(incorporationDate)) {
            throw new TemporalValidationException(ErrorCatalog.ERR_COMPANY_MISSING_INCORPORATION_DATE, EntityContext.COMPANY);
        }
        if (incorporationDate.isAfter(LocalDate.now())) {
            throw new TemporalValidationException(ErrorCatalog.ERR_COMPANY_FUTURE_INCORPORATION_DATE, EntityContext.COMPANY);
        }
