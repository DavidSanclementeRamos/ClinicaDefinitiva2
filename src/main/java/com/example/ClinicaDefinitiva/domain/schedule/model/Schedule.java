...
public final class Schedule {
    ...
    public void validateScheduleBetween(LocalDateTime start, LocalDateTime end) {
        if (start == null) {
            throw new AppointmentStartDateMissingException(ContextoEntidad.APPOINTMENT,"La fecha de inicio no puede ser null");
        }
        if (end == null) {
            throw new AppointmentEndDateMissingException(ContextoEntidad.APPOINTMENT, "La fecha de fin no puede ser nula.");
        }
        if (!start.isBefore(end)) {
            throw new AppointmentInvalidDateRangeException(ContextoEntidad.APPOINTMENT,"El rango de fechas es inválido: inicio" + start + " no es anterior a fin" + end);
        }
        ...
        if (!coveredBySlot) {
            throw new AppointmentOutsideAvailabilityException(ContextoEntidad.APPOINTMENT, "El intervalo no está cubierto por la disponibilidad declarada:" + start + " y " + end);
        }
        ...
        if (!slotFree) {
            throw new AppointmentTimeNotAvailableException(ContextoEntidad.APPOINTMENT, "El intervalo ya está ocupado por otra cita: " + start+ " y " + end);
        }
    }
