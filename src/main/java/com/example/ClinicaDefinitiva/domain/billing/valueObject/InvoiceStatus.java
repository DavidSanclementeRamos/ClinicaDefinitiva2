...
public final class InvoiceStatus {
    ...
    private InvoiceStatus(Status value) {
        if (value == null) {
            throw new ValueObjectValidationException(
                BillingVOError.ERR_INVOICE_STATUS_NULL,
                VOContext.BILLING
            );
        }
        this.value = value;
    ...
    public static InvoiceStatus draft() { return new InvoiceStatus(Status.DRAFT); }
    public static InvoiceStatus pending() { return new InvoiceStatus(Status.PENDING); }
    public static InvoiceStatus paid() { return new InvoiceStatus(Status.PAID); }
    public static InvoiceStatus cancelled() { return new InvoiceStatus(Status.CANCELLED); }
    public static InvoiceStatus of(Status status) { return new InvoiceStatus(status); }

    public boolean canTransitionTo(Status next) {
        Set<Status> allowedTransitions = VALID_TRANSITIONS.getOrDefault(this.value, EnumSet.noneOf(Status.class));
    ...
    public InvoiceStatus transitionTo(Status next) {
        if (!canTransitionTo(next)) {
            throw new ValueObjectValidationException(
                BillingVOError.ERR_INVOICE_INVALID_STATUS_TRANSITION,
                VOContext.BILLING
            );
        }
    ...
    public boolean isDraft() { return this.value == Status.DRAFT; }
    public boolean isPending() { return this.value == Status.PENDING; }
    public boolean isPaid() { return this.value == Status.PAID; }
    public boolean isCancelled() { return this.value == Status.CANCELLED; }
    public boolean isEditable() { return isDraft() || isPending(); }
