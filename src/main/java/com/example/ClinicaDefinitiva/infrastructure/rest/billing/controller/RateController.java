...
import com.example.ClinicaDefinitiva.application.billing.input.RateUseCase;
...
import com.example.ClinicaDefinitiva.infrastructure.rest.billing.mapper.rate.RateRestReadMapper;
import com.example.ClinicaDefinitiva.infrastructure.rest.billing.mapper.rate.RateRestWriteMapper;
...
public class RateController {
    ...
    public ResponseEntity<Page<PageRateResponse>> findByContract(
        ...
        Page<PageRateDto> rates = useCase.findByContract(ContractId.of( contractId), pageable, userDetails.getId(), userDetails.getActiveRolId());
        return ResponseEntity.ok(rates.map(readMapper::toPageRest));
    }
    ...
    public ResponseEntity<ReadRateResponse> findActiveRateForService(
        ...
        ReadRateDto dto = useCase.findActiveRateForService(ServiceId.of( serviceId), payerType,ContractId.of(contractId) ,
    ...
    public ResponseEntity<ReadRateResponse> endValidity(
        ...
        ReadRateDto updated = useCase.endValidityAt(RateId.of(id), endDate,
                userDetails.getId(), userDetails.getActiveRolId());
